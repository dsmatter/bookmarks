...
var changeBookmark = function(element, newTitle, newUrl, oldTitle, oldUrl) {
  ...
  ajax(element, '/bookmark/' + bookmarkId, 'POST', { title: newTitle, url: newUrl }).done(function(bookmarkHtml) {
    ...
    element.find('.link').html('<a href="' + oldUrl + '">' + oldTitle + ' target="_blank"</a>');
...
var setupEditModeForBookmark = function(element) {  
  ...
  element.find('.link').html('<input class="title" type="text" value="' + oldTitle + tags + '" /><input class="url" type="text" value="' + oldLUrl + '" />');
  element.find('input').keypress(function(event) {
    if (event.which == 13) {
