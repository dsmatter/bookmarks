...
var setupEditModeForBookmark = function(element) {  
  var oldTitle = element.find('.link a').text();
  var oldLUrl = element.find('.link a').attr('href');
...
var setupList = function(element) {
  ...
  element.droppable({
    drop: function(e, ui) {
      var bookmarkId = ui.draggable.attr('id').replace('bookmark-', '');
      var listElement = $(this);

      ajax(ui.draggable, '/bookmark/' + bookmarkId + '/move/' + listId).done(function(oldListHtml) {
        var oldListId = $(oldListHtml).attr('id').replace('list-', '');
        $('#list-' + oldListId).replaceWith(oldListHtml);
        var newOldList = $('#list-' + oldListId);
...
var setupNewest = function() {
  $('#newest .bookmark').attr('id', '');
  $('#newest .actions').remove()
};
...
var filterBookmarks = function(text) {
  ...
  $('.list').each(function() {
    ...
    $(this).find('.bookmarks li').each(function() {
      // Exclude tags and no bookmarks banner
      if ($(this).parent().parent().hasClass('tags') || $(this).hasClass('hi')) {
        return;
      }
